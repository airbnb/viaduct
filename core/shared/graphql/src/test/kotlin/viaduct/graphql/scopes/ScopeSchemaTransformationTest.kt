...
class ScopeSchemaTransformationTest : SchemaScopeTestBase() {
    ...
    @Test
    fun `drops implemented interfaces when scoped fields cannot satisfy the interface contract`() {
        val schema = schemaFromSdl(
            """
    ...
    @Test
    fun `drops implemented interfaces when scoped fields have incompatible types`() {
        val schema = schemaFromSdl(
            """
            type Query @scope(to: ["public", "private"]) {
            ...
            interface App @scope(to: ["public", "private"]) {
                id: ID!
                secret: String
            }

            type Client implements App @scope(to: ["public", "private"]) {
                id: ID!
            }

            extend type Client @scope(to: ["private"]) {
                secret: String
            }

            interface Authorization @scope(to: ["public", "private"]) {
                app: App
            }

            type ClientAuthorization implements Authorization @scope(to: ["public", "private"]) {
                app: Client
            }
            """.trimIndent()
        )

        val publicAuthorization = ScopedSchemaBuilder(schema, sortedSetOf("public", "private"), listOf())
            .applyScopes(setOf("public"))
            .filtered
            .getObjectType("ClientAuthorization")

        assertEquals(emptyList<String>(), publicAuthorization.interfaces.map { it.name })
    }
