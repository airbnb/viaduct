 ...
 */
class RrsServer(private val config: RrsConfiguration) {
    private val log = LoggerFactory.getLogger(RrsServer::class.java)
    ...
    fun start() {
        ...
        log.info("Starting RRS gRPC server on port {}", config.port)
        try {
        ...
        log.info(
            "RRS gRPC server listening on port {} (RRP callback expected at {}:{})",
            config.port,
            config.callbackHost,
            config.callbackPort
        )
    ...
    fun stop() {
        ...
        val s = server ?: return
        log.info("Shutting down RRS gRPC server")
        s.shutdown()
        try {
