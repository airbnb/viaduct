...
We've included a field `throwException` to demonstrate GraphQL error handling in Viaduct. If you add that field to your query and hit the play button, you'll see a different result:
...
As this example demonstrates, to support more fault-tolerant applications, GraphQL errors typically do _not_ cause an entire request to fail. Instead, they are contained as much as possible to the fields that actually failed.
...
And `ThrowExceptionResolver.kt` contains:
...
Next, add a new resolver to `resolvers/src/main/kotlin/com/example/viadapp/resolvers/HelloWorldResolver.kt`:
...
The parameter provided to the `@Resolver` annotation is a central Viaduct feature. It declares that this resolver depends on the existing `greeting` and `author` fields. Given this annotation, Viaduct will make those values available through `ctx.objectValue` before your resolver runs.
...
If you are evaluating Viaduct for real adoption, the next question is usually not just "can I write a resolver?" but "how hard is this to fit into my stack?" The `ktor-starter` is intentionally small so you can answer that quickly.

### The Ktor host

`src/main/kotlin/com/example/viadapp/ViaductApplication.kt` is the Ktor entry point: it contains the server's `main` function and configures `io.ktor.server.application.Application`.
