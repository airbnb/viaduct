...
object GradleTestVersions {
    ...
    private val upperLimitExclusive = GradleVersion.version("9.0")

    private val lowerLimitInclusive = GradleVersion.version("8.10")

    internal
    fun getVersions(): List<String> {
        val releasedVersions = getReleasedVersions()
        val testVersions = keepOnlyLatestMinor(releasedVersions).toMutableList()
    ...
    private fun getReleasedVersions(): List<VersionInfo> {
        ...
        val allVersions = Gson().fromJson(jsonText, Array<VersionJsonBlock>::class.java)
            ...
            .filter { it.gradleVersion >= lowerLimitInclusive }
            .filter { it.gradleVersion < upperLimitExclusive }
            .toList()
        return allVersions
    }
