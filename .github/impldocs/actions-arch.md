...
The first three are composed by `ci-manual-trigger.yml` — they don't have direct `push` or `pull_request` triggers themselves.
...
| Workflow | Purpose | Runs on |
|---|---|---|
| `ci-manual-trigger.yml` | Full CI suite: build-and-test + demoapp tests + API compat check | PR, merge to main, daily schedule (via periodic_green_check), manual |
| `periodic_green_check.yml` | Scheduled CI health check + branch staleness detection | daily schedule, manual |
...
schedule
  |
  v
periodic_green_check.yml  [orchestrator]
  |
  |--- ci-check
  |      |
  |      v
...
| `periodic_green_check.yml` | Run scheduled checks without waiting for cron | `branch`, `mode` (ci-check / staleness-check / all) |
