...
# - Arguments: `--base-dir`, `--run-id`, `--repository`, and `--server-url`.
# - Behavior: scan `--base-dir` recursively for build-scan JSON artifacts, collect labeled scan
#   URLs plus the first usable PR number, then print a JSON payload for the workflow-side comment
#   posting step. The payload always includes `comment_marker`; `pr_number` and `body` may be null
#   when there is nothing to post.
# - Error handling: malformed JSON artifact files are skipped so one bad file does not block comment
#   generation. Other exceptions are caught at process top level, printed as `Error: ...` on stderr,
#   and returned as exit status 1.
# - Non-zero exit for callers means comment preparation itself failed and the workflow should treat
#   that as an infrastructure/configuration problem, not as "there were simply no build scans".
